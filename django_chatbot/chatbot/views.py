...
def chatbot(request):
    try:
        ...
        if prompt:
            response, sources = ask_openai(prompt, request)
            sources = json.loads(sources)

            chat = Chat(user=request.user, message=prompt, response=response, created_at=timezone.now())
            chat.save()

            # Pass initial response data to the template for immediate display
